...
class ProductService
{
    ...
    {
        $product = Product::findOrFail($this->product->id);
        $product->subcategories()->sync($this->convertToNumericArray($subCategories));
    }

    public function attachProductBrands($brands)
    {
        $product = Product::findOrFail($this->product->id);
        $product->brands()->sync($this->convertToNumericArray($brands));
    }

    private function convertToNumericArray($array):array{
